...
class Process:
    ...
    def predict(self):
        ...
        # export_feature.to_csv(f'{self.randomize}\\MODEL{self.model_name}RANDOMIZE{self.randomize}NOISE{self.randomize_value}SEED{self.random_seed}.csv',index=False)

        all_PDP_df = pd.DataFrame()

        for col in self.original_df_validation_feature.columns:

            print(col)
            
            PDP_df_col = pd.DataFrame()

            feature_range = [x*0.1 for x in range(11)]

            for feature_value in feature_range:
                
                PDP_df = pd.DataFrame([[0.5,0.5,0.5,0.5,0.5,0.5,0.5]],columns=self.original_df_validation_feature.columns)
                PDP_df[col] = feature_value

                PDP_prediction = pd.DataFrame(self.current_model.predict(PDP_df),columns=[col])

                PDP_df_col = pd.concat([PDP_df_col,PDP_prediction],axis='rows')

            all_PDP_df = pd.concat([all_PDP_df,PDP_df_col],axis='columns')

        all_PDP_df['FEATURE_VALUE'] = feature_range
        all_PDP_df = all_PDP_df.reset_index(drop=True)
        all_PDP_df.to_csv(f'{randomize}\\PDPMODEL{model_name}RANDOM{randomize}VALUE{self.randomize_value}.csv',index=False)
...
class Stacking_Process:
    ...
    def process(self):
        ...
        for random_seed in range(1):
            ...
            for count,randomize_value in enumerate(self.randomize_value_list):
                ...
                if self.has_categorical == True:

                    self.separate_categorical_and_numerical_data()
                    self.randomize_categorical_data()
                    self.encode_categorical_data()

                    self.randomize_numerical_data()
                    self.split_numerical_data()
                    self.scale_numerical_data()

                    self.combine_categorical_and_numerical()
        ...
        for count_for_train, (train_index_for_validation,test_index_for_validation) in enumerate(self.splits_for_training):
            ...
            for count,model_name in enumerate(self.stack_model_tuple):
                ...
                try:
                    ...
                    if self.stack_model_class_tuple[count] == LGBMRegressor:
    ...
    def predict(self):
        ...
        # export_feature.to_csv(f'{self.randomize}\\MODELSTACKRANDOMIZE{self.randomize}NOISE{self.randomize_value}SEED{self.random_seed}.csv',index=False)
        ...
        for col in self.original_df_validation_feature.columns:
            ...
            for feature_value in feature_range:
                ...
                for count,refit_model in enumerate(self.refit_base_model_dict):
                    ...
                    refit_model_prediction = pd.DataFrame(self.refit_base_model_dict[refit_model]['model'].predict(PDP_df),columns=[f'MODEL {count}'])
                    self.META_prediction_df = pd.concat([self.META_prediction_df,refit_model_prediction],axis='columns')

                self.META_prediction = pd.DataFrame(self.META_model.predict(self.META_prediction_df),columns=[col])

                PDP_df_col = pd.concat([PDP_df_col,self.META_prediction],axis='rows')

            all_PDP_df = pd.concat([all_PDP_df,PDP_df_col],axis='columns')

        all_PDP_df['FEATURE_VALUE'] = feature_range
        all_PDP_df = all_PDP_df.reset_index(drop=True)
        all_PDP_df.to_csv(f'{randomize}\\PDPMODELSTACKRANDOM{randomize}VALUE{self.randomize_value}.csv',index=False)
...
for randomize in randomize_list:
    ...
    # model_df.to_csv(f'{randomize}STACK.csv',index=False)
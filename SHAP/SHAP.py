...
for count in range(len(model_list)):
    ...
    model_df.to_csv(f'CSV_SHAP\\SHAP{model_name}0.csv',index=False)
...
model_df.to_csv(f'CSV_SHAP\\SHAPSTACK0.csv',index=False)
...
#         self.randomize_value_list = [x for x in np.arange(0,0.61,0.075)]
...
#         explainer = shap.KernelExplainer(self.current_model.predict,self.df_train_feature.head(20))
#         shap_values = explainer.shap_values(self.df_validation_feature.head(20))
#         SHAP_key_name = f'{self.model_name}value{self.randomize_value:.2f}seed{self.random_seed}'
#         self.shap_value_dict[SHAP_key_name] = shap_values

#         print(self.shap_value_dict)
